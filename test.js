...
console.log(
  "- BLOB_RETURN_CODES:",
  Object.keys(BLOB_RETURN_CODES).length,
  "codes"
);
...
console.log(
  "- RETURN_DEVICE_NOT_AVAILABLE:",
  RETURN_CODES.RETURN_DEVICE_NOT_AVAILABLE
);
...
try {
  const errorMsg = tmgInstance.getErrorMessage(
    RETURN_CODES.RETURN_DEVICE_NOT_AVAILABLE
  );
  console.log("✅ getErrorMessage() works:", errorMsg);

  const blobErrorMsg = tmgInstance.getBlobErrorMessage(
    BLOB_RETURN_CODES.BLOB_RET_ERROR_BUSY
  );
