...
const { API_ERROR_CODES } = require("../utils/constants");
...
function authenticateApiKey(req, res, next) {
  ...
  if (apiKey !== validApiKey) {
    logger.warn(
      `Invalid API key attempt from ${req.ip}: ${apiKey.substring(0, 8)}...`
    );
    const error = createApiError(
      "Invalid API key",
...
function getUserRole(req) {
  ...
  if (headerRole && Object.values(ROLES).includes(headerRole)) {
    return headerRole;
  }
...
const requireReadAccess = requirePermission("read_devices");
const requireWriteAccess = requirePermission("write_parameters");
const requireOperatorAccess = requirePermission("write_process_data");
const requireAdminAccess = requirePermission("connect_master");
