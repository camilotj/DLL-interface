 ...
 */
function validatePortNumber(req: Request, res: Response, next: NextFunction): void {
 ...
 */
function validateMasterExists(req: Request, res: Response, next: NextFunction): void {
  const masterHandle = parseInt(
    (req.params as any).masterHandle || req.body.masterHandle
  );

  if (isNaN(masterHandle) || masterHandle < 0) {
    const error: ValidationError = new Error('Valid master handle is required') as ValidationError;
    error.isJoi = true;
    return next(error);
  }
 ...
 */
function validateProcessDataLength(req: Request, res: Response, next: NextFunction): void {
  ...
  if (length > 32) {
    const error: ValidationError = new Error('Process data cannot exceed 32 bytes') as ValidationError;
    error.isJoi = true;
    return next(error);
  }
 ...
 */
function validateParameterValue(req: Request, res: Response, next: NextFunction): void {
