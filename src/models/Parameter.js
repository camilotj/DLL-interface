...
class Parameter {
  constructor({
    ...
    defaultValue = null,
    unit = "",
    isStandard = false, // true for standard IO-Link parameters
  }) {
    this.index = index;
    this.subIndex = subIndex;
    this.name = name;
    this.description = description;
    this.dataType = dataType; // uint8, uint16, uint32, int8, int16, int32, string, bytes
    this.access = access;
    this.length = length;
  ...
  validateValue(value) {
    ...
    if (
      (this.dataType === "string" || this.dataType === "bytes") &&
      this.length > 0
    ) {
      const actualLength =
        value.length || (Buffer.isBuffer(value) ? value.length : 0);
      if (actualLength > this.length) {
        errors.push(
          `Value too long. Maximum length: ${this.length}, actual: ${actualLength}`
        );
      }
    }
  ...
  parseValue(buffer) {
    ...
    try {
      switch (this.dataType) {
        ...
        case "string":
          return buffer.toString("utf8").replace(/\0+$/, ""); // Remove null terminators
