...
class Device {
  constructor({
    ...
    this.connectionState = "DISCONNECTED"; // DISCONNECTED, PREOPERATE, OPERATE, WRONG_DEVICE
    this.actualMode = 0;
    this.sensorStatus = 0;
    this.baudrate = 0;
    ...
    // Device capabilities
    this.supportedParameters = new Map(); // index -> parameter info
    this.processDataCache = null;
    this.parameterCache = new Map(); // "index.subindex" -> cached value
  ...
  isParameterCacheValid(index, subIndex, maxAgeMs = 30000) {
    ...
    const age = new Date() - cached.cachedAt;
    return age < maxAgeMs;
  }
  ...
  isProcessDataCacheValid(maxAgeMs = 1000) {
    ...
    const age = new Date() - this.processDataCache.timestamp;
    return age < maxAgeMs;
  }
  ...
  getConnectionStatus() {
    return {
      ...
      connectedAt: this.connectedAt,
      lastSeen: this.lastSeen,
      uptime: this.connectedAt ? new Date() - this.connectedAt : 0,
    };
  }
