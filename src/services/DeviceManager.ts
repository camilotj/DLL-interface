...
class DeviceManager {
  private iolinkService: IOLinkService;
  private connectedMasters: Map<number, MasterInfo>;
  private devices: Map<string, Device>;
  private deviceSubscriptions: Map<string, any>;
  public parameters: Map<string, Map<string, Parameter>>;
  private scanIntervals: Map<number, NodeJS.Timeout>;
  private monitoringEnabled: boolean;
  private monitoringInterval: NodeJS.Timeout | null;

  constructor() {
    this.iolinkService = new IOLinkService();
  ...
  async connectMaster(deviceName: string): Promise<number> {
    try {
      ...
      for (const [handle, master] of this.connectedMasters) {
        if (master.deviceName === deviceName) {
          logger.warn(
            `Master ${deviceName} already connected with handle ${handle}`
          );
          return handle;
        }
      }
      ...
      logger.info(
        `Master ${deviceName} connected successfully with handle ${handle}`
      );
      return handle;
    } catch (error: any) {
      logger.error(`Failed to connect master ${deviceName}:`, error.message);
      throw error;
    }
  }

  async disconnectMaster(handle: number): Promise<boolean> {
    try {
      ...
      logger.info(`Disconnecting master: ${masterInfo.deviceName}`);
      ...
      logger.info(`Master ${masterInfo.deviceName} disconnected successfully`);
      return true;
    } catch (error: any) {
      logger.error(`Failed to disconnect master:`, error.message);
      throw error;
    }
  }
  ...
  getConnectedMasters(): any[] {
    ...
    for (const [handle, masterInfo] of this.connectedMasters) {
      masters.push({
        ...
        deviceCount: Array.from(this.devices.values()).filter(
          (d) => d.masterHandle === handle
        ).length,
      });
    }
    return masters;
  ...
  async startDeviceScanning(
    ...
    const scanDevices = async () => {
      try {
        ...
        logger.error(
          `Device scanning error for master ${masterHandle}:`,
          error.message
        );
      }
    };
    ...
    logger.info(
      `Started device scanning for master ${masterHandle} (interval: ${intervalMs}ms)`
    );
  }
  ...
  stopDeviceScanning(masterHandle: number): void {
    ...
    if (intervalId) {
      ...
      logger.info(`Stopped device scanning for master ${masterHandle}`);
    }
  }

  async scanDevicesOnMaster(masterHandle: number): Promise<void> {
    const masterInfo = this.connectedMasters.get(masterHandle);
    if (!masterInfo) {
      throw new Error(`Master with handle ${masterHandle} not connected`);
    }

    logger.debug(`Scanning devices on master ${masterInfo.deviceName}`);

    for (let port = 1; port <= LIMITS.MAX_PORTS; port++) {
      try {
        const status = await this.iolinkService.checkPortStatus(
          masterHandle,
          port
        );
        ...
        if (status.connected) {
          if (!existingDevice) {
            ...
            logger.debug(
              `Updated device status for port ${port}: ${status.mode}`
            );
          }
        } else {
          if (existingDevice) {
        ...
        logger.debug(
          `Error checking port ${port} on master ${masterHandle}:`,
          error.message
        );
      }
    }
  ...
  async handleNewDeviceDetected(
    ...
    try {
      ...
      const deviceInfo = (this.iolinkService as any).parseDeviceInfoFromDPP(
        status.directParameterPage,
        port
      );

      if (!deviceInfo) {
        logger.warn(`Could not parse device info for port ${port}`);
        return;
      }
      ...
      logger.info(
        `Device registered: ${device.vendorName} ${device.deviceName} on port ${port}`
      );

      return device;
    } catch (error: any) {
      logger.error(`Error handling new device on port ${port}:`, error.message);
    }
  }
  ...
  async handleDeviceDisconnected(
    ...
    setTimeout(() => {
      if (!device.connected) {
        this.devices.delete(deviceKey);
        this.parameters.delete(deviceKey);
        logger.debug(`Removed disconnected device from registry: ${deviceKey}`);
      }
    }, 30000);
  }
  ...
  async initializeDeviceParameters(
    ...
    for (const [index, definition] of Object.entries(STANDARD_PARAMETERS)) {
      const parameter = Parameter.createStandardParameter(
        parseInt(index),
        definition as any
      );
    ...
    logger.debug(
      `Initialized ${parameterMap.size} standard parameters for device ${deviceKey}`
    );
  }

  async readDeviceMetadata(deviceKey: string, device: Device): Promise<void> {
    try {
      const tasks = [
        this.readDeviceParameter(deviceKey, PARAMETER_INDEX.SERIAL_NUMBER),
        this.readDeviceParameter(deviceKey, PARAMETER_INDEX.FIRMWARE_REVISION),
        this.readDeviceParameter(deviceKey, PARAMETER_INDEX.HARDWARE_REVISION),
        this.readDeviceParameter(
          deviceKey,
      ...
      logger.debug(`Updated metadata for device ${deviceKey}`);
    } catch (error: any) {
      logger.debug(
        `Could not read all metadata for device ${deviceKey}:`,
        error.message
      );
    }
  }
  ...
  async getDeviceStatus(masterHandle: number, port: number): Promise<any> {
  ...
  async readProcessData(masterHandle: number, port: number): Promise<any> {
    const device = this.getDevice(masterHandle, port);

    if (!device.isReady()) {
      throw new Error(
        `Device on port ${port} is not ready for process data operations`
      );
    }

    // Check cache first
    if (device.isProcessDataCacheValid(LIMITS.CACHE_TTL_PROCESS_DATA)) {
      logger.debug(`Returning cached process data for port ${port}`);
    ...
    logger.debug(
      `Read process data from port ${port}: ${result.data.length} bytes`
    );
    return result;
  }
  ...
  async writeProcessData(
    ...
    const result = await this.iolinkService.writeProcessData(
      masterHandle,
      port,
      data
    );
    logger.debug(
      `Wrote process data to port ${port}: ${result.bytesWritten} bytes`
    );
    return result;
  }
  ...
  async readDeviceParameter(
    ...
    if (
      parameter &&
      device.isParameterCacheValid(index, subIndex, LIMITS.CACHE_TTL_PARAMETERS)
    ) {
      logger.debug(
        `Returning cached parameter ${parameterId} for device ${deviceKey}`
      );
      return device.getCachedParameter(index, subIndex);
    }

    const result = await this.iolinkService.readParameter(
      device.masterHandle!,
      device.port,
      index,
      subIndex
    ...
    logger.debug(`Read parameter ${parameterId} from device ${deviceKey}`);
    return result;
  }
  ...
  async writeDeviceParameter(
    ...
    const result = await this.iolinkService.writeParameter(
      device.masterHandle!,
      device.port,
      index,
      subIndex,
    ...
    logger.debug(`Wrote parameter ${parameterId} to device ${deviceKey}`);
    return result;
  }
  ...
  async validateDeviceConnection(
    ...
    if (!device.isInCommunication()) {
      throw new Error(
        `Device on master ${masterHandle} port ${port} is not connected`
      );
    }
    return device;
  }
  ...
  async cleanup(): Promise<void> {
    ...
    for (const [handle, masterInfo] of this.connectedMasters) {
      try {
        ...
        logger.error(
          `Error disconnecting master ${masterInfo.deviceName}:`,
          error.message
        );
      }
    }
