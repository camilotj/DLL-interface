...
const IOLinkService = require("./IOLinkService");
...
class DeviceManager {
  constructor() {
    ...
    this.connectedMasters = new Map(); // handle -> master info
    this.devices = new Map(); // "masterHandle:port" -> Device instance
    this.deviceSubscriptions = new Map(); // device ID -> subscription info
    this.parameters = new Map(); // device ID -> Map of parameters

    // Scanning and monitoring
    this.scanIntervals = new Map(); // master handle -> interval ID
  ...
  async startDeviceScanning(masterHandle, intervalMs = 5000) {
  ...
  async handleNewDeviceDetected(masterHandle, port, status) {
    try {
      ...
      const deviceInfo = this.iolinkService.parseDeviceInfoFromDPP(
  ...
  async handleDeviceDisconnected(deviceKey, device) {
    ...
    // Remove it completely after a timeout
    setTimeout(() => {
      if (!device.connected) {
        this.devices.delete(deviceKey);
        this.parameters.delete(deviceKey);
  ...
  async readDeviceParameter(deviceKey, index, subIndex = 0) {
    ...
    if (
      parameter &&
      parameter.isParameterCacheValid(
        index,
        subIndex,
        LIMITS.CACHE_TTL_PARAMETERS
      )
    ) {
      logger.debug(
        `Returning cached parameter ${parameterId} for device ${deviceKey}`
      );
      return parameter.getCachedParameter(index, subIndex);
    }
