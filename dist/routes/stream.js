 ...
 */
router.get('/status', auth_1.requireReadAccess, (req, res) => {
 ...
 */
router.get('/active', auth_1.requireReadAccess, (req, res) => {
    const streamController = require('../controllers/streamController');
    const streams = Array.from(streamController.activeStreams.entries()).map((entry) => {
 ...
 */
router.get('/:masterHandle/:deviceId/device', auth_1.requireReadAccess, validation_1.validateMasterHandle, validation_1.validateDeviceId, auth_1.authorizeDeviceAccess, (req, res) => {
    ...
    res.json({
        ...
        data: {
            ...
            instructions: 'Connect to WebSocket and emit "subscribe:device" with the subscription payload',
        },
    });
});
/**
 ...
 */
router.get('/:masterHandle/:deviceId/process-data', auth_1.requireReadAccess, validation_1.validateMasterHandle, validation_1.validateDeviceId, auth_1.authorizeDeviceAccess, (req, res) => {
    const { masterHandle, deviceId } = req.params;
    const deviceKey = `${masterHandle}:${deviceId}`;
    res.json({
        success: true,
        data: {
            ...
            instructions: 'Connect to WebSocket and emit "subscribe:process-data" with the subscription payload',
        },
    });
});
/**
 ...
 */
router.get('/:masterHandle/:deviceId/parameters/:index', auth_1.requireReadAccess, validation_1.validateMasterHandle, validation_1.validateDeviceId, auth_1.authorizeDeviceAccess, (req, res) => {
    ...
    res.json({
        ...
        data: {
            ...
            instructions: 'Connect to WebSocket and emit "subscribe:parameter" with the subscription payload',
        },
    });
});
// ============================================================================
        ...
        data: {
            ...
            examples: {
                javascriptClient: {
                    ...
                    subscribeDevice: "socket.emit('subscribe:device', { masterHandle: 1, deviceId: 1, interval: 1000 });",
                    subscribeParameter: "socket.emit('subscribe:parameter', { masterHandle: 1, deviceId: 1, parameterIndex: 15, interval: 5000 });",
                    handleData: "socket.on('device:data', (data) => { console.log('Device data:', data); });",
                    unsubscribe: "socket.emit('unsubscribe', { type: 'device', deviceKey: '1:1' });",
                },
            },
        },
    });
