...
class IOLinkService {
    ...
    async connectToMaster(deviceName) {
        try {
            ...
            logger_1.default.info(`Successfully connected and initialized master ${deviceName} with handle ${handle}`);
            return handle;
        }
        catch (error) {
    ...
    async resetMaster(handle) {
        logger_1.default.info(`Resetting master state for handle ${handle}...`);
        try {
            ...
            for (let port = 0; port < 2; port++) {
                ...
                try {
                    ...
                    logger_1.default.debug(`Port ${port + 1}: Reset result = ${clearResult}`);
                }
                catch (portError) {
                    logger_1.default.debug(`Port ${port + 1}: Reset failed - ${portError.message}`);
                }
            }
            logger_1.default.info(`Master reset complete, waiting for stabilization...`);
            // Wait for hardware stabilization after reset
            await new Promise((resolve) => setTimeout(resolve, 2000));
            return true;
        }
        catch (error) {
            logger_1.default.error(`Master reset failed: ${error.message}`);
            return false;
        }
    }
    async initializeMaster(handle, deviceName, maxPorts = 2) {
        logger_1.default.info(`Initializing IO-Link Master: ${deviceName}`);
        const masterState = this.masterStates.get(handle);
        if (!masterState) {
            throw new Error(`Master state not found for handle ${handle}`);
        }
        logger_1.default.info(`Configuring ${maxPorts} ports for IO-Link operation...`);
        for (let port = 1; port <= maxPorts; port++) {
            try {
                const configSuccess = await this.configurePortForIOLink(handle, port);
                if (configSuccess) {
                    logger_1.default.info(`Port ${port}: Configured for IO-Link operation`);
                }
                else {
                    logger_1.default.debug(`Port ${port}: Configuration failed or port does not exist`);
                }
            }
            catch (error) {
                logger_1.default.error(`Port ${port}: Configuration error: ${error.message}`);
            }
        }
        masterState.initialized = true;
        masterState.configurationComplete = true;
        logger_1.default.info("Waiting for port stabilization (IO-Link timing requirements)...");
        await new Promise((resolve) => setTimeout(resolve, 5000));
        logger_1.default.info("Additional device detection wait...");
        await new Promise((resolve) => setTimeout(resolve, 3000));
        logger_1.default.info(`Master ${deviceName} initialization complete`);
    }
    async configurePortForIOLink(handle, port) {
        try {
            const zeroBasedPort = port - 1; // 0-based for DLL
            logger_1.default.debug(`Port ${port}: Checking current configuration state...`);
            ...
            if (checkResult === constants_1.RETURN_CODES.RETURN_WRONG_PARAMETER) {
                return false; // Port doesn't exist
            }
            logger_1.default.debug(`Port ${port}: Current config - TargetMode=${currentConfig.TargetMode}, CRID=0x${currentConfig.CRID.toString(16)}`);
            // Create IO-Link port configuration
            const portConfig = new TPortConfiguration();
            portConfig.PortModeDetails = 0;
            portConfig.TargetMode = constants_1.PORT_MODES.SM_MODE_IOLINK_OPERATE;
            ...
            logger_1.default.debug(`Port ${port}: Setting config - CRID=0x${portConfig.CRID.toString(16)}, TargetMode=${portConfig.TargetMode}, InspectionLevel=${portConfig.InspectionLevel}`);
            ...
            logger_1.default.debug(`Port ${port}: IOL_SetPortConfig result = ${result} (SUCCESS)`);
            return true;
        }
        catch (error) {
            logger_1.default.error(`Failed to configure port ${port}: ${error.message}`);
            return false;
        }
    }
